...
async function validateUserLogin(req, res) {
  try {
    ...
    if (token.error || token.Error || token.Message) {
      logErrorToDB("UserController", "validateUserLogin", `Inicio de sesion fallido: ${user.email}`, "");
    }
    res.json({ token });
  } catch (error) {
    logErrorToDB("UserController", "validateUserLogin", error.message, error.stack);
