...
async function getAll(req, res) {
    const { clientId } = req.query; 
        ...
        if (clientId && isNaN(parsedId)) {
           await  logErrorToDB("OrderController","getAll",`El clientId no es un número válido: ${clientId}`, "");
...
async function getById(req, res) {
    ...
    try {
        ...
        if (!order) {
           ...
           await  logErrorToDB("OrderController","getById",order.error||order.Error||`No se encontró la orden con ID ${id}`, "");
            return res.status(404).json({ error: "Orden no encontrada" });
        }
        res.status(200).json(order);
    } catch (error) {
       await  logErrorToDB("OrderController","getById",error.message, error.stack);
        res.status(500).json({ error: error.message });
    }
}
...
async function create(req, res) {
    try {
        ...
        if (!newOrder) {
           await  logErrorToDB("OrderController","create",newOrder.error||newOrder.Error||"Error al crear la orden", "");
       ...
       await  logErrorToDB("OrderController","create",error.message, error.stack);
...
async function addProductToOrder(req, res) {
    try {
        ...
        if (result.Message) {
           ...
           await  logErrorToDB("OrderController","addProductToOrder",result.error||result.ERROR||result.Error||"Error al agregar el producto a la orden", "");
            res.status(400).json({ error: result.error || 'Error al agregar el producto a la orden' });
        }
    } catch (error) {
       await  logErrorToDB("OrderController","addProductToOrder",error.message, error.stack);
...
async function getNextOrderId(req, res) {
    try {
       ...
       await  logErrorToDB('OrderController', 'getNextOrderId', error.message, error.stack);
